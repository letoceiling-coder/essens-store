...
class SetDeploy extends Command
{
    ...
    public function handle()
    {
        ...
        // Проверка и установка remote URL
        $expectedRemoteUrl = 'https://github.com/letoceiling-coder/essens-store.git';
        $this->newLine();
        $this->info('🔗 Проверка remote URL...');
        
        $remoteUrlProcess = new SymfonyProcess(['git', 'remote', 'get-url', 'origin']);
        $remoteUrlProcess->run();
        $currentRemoteUrl = trim($remoteUrlProcess->getOutput());
        
        if (empty($currentRemoteUrl)) {
            $this->warn('⚠️  Remote origin не настроен. Устанавливаем...');
            $setRemoteProcess = new SymfonyProcess(['git', 'remote', 'add', 'origin', $expectedRemoteUrl]);
            $setRemoteProcess->run();
            if ($setRemoteProcess->isSuccessful()) {
                $this->info('✅ Remote origin установлен: ' . $expectedRemoteUrl);
            } else {
                $this->error('❌ Ошибка при установке remote: ' . $setRemoteProcess->getErrorOutput());
                return Command::FAILURE;
            }
        } elseif ($currentRemoteUrl !== $expectedRemoteUrl) {
            $this->warn("⚠️  Remote URL отличается от ожидаемого:");
            $this->line("   Текущий: {$currentRemoteUrl}");
            $this->line("   Ожидаемый: {$expectedRemoteUrl}");
            if ($this->confirm('Изменить remote URL на правильный?', true)) {
                $setRemoteProcess = new SymfonyProcess(['git', 'remote', 'set-url', 'origin', $expectedRemoteUrl]);
                $setRemoteProcess->run();
                if ($setRemoteProcess->isSuccessful()) {
                    $this->info('✅ Remote URL обновлен');
                } else {
                    $this->error('❌ Ошибка при обновлении remote: ' . $setRemoteProcess->getErrorOutput());
                    return Command::FAILURE;
                }
            }
        } else {
            $this->info('✅ Remote URL настроен правильно: ' . $expectedRemoteUrl);
        }
